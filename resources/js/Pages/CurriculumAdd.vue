...
export default {
    ...
    mounted() {
        this.selectlist = this.response.filter(item => item.select == false);
    ...
    methods: {
        ...
        curriculumSave () {
            this.$inertia.post('/curriculum-save',{
                ...
                <div class="flex flex-col gap-3 p-5 bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="flex justify-between gap-2">
                        ...
                        <div class="w-1/2">
                            ...
                            <input v-model="term" type="text" id="teacher-introduction" class="w-full">
                        </div>
                    </div>
                    <div class="mb-5 flex justify-center gap-2">
                        <div class="w-2/5 min-h-[300px] rounded-lg overflow-hidden border border-black">
                            <table class="w-full text-center">
                                ...
                                <tbody>
                                    <tr>
                                        <label v-for="item in noselectlist" :for="item.id" :key="item.id" class="grid grid-cols-2 border-b">
                                            <td :class="{'bg-blue-500' : item.select}" class="flex justify-center items-center col-span-1">{{ item.course.course_name }}
                                            </td>
                                            <td :class="{'bg-blue-500' : item.select}" class="flex justify-center items-center col-span-1">{{ item.teacher_name }}
                                            </td>
                                            <input v-model="item.select" type="checkbox" :id="item.id" class="hidden">
                                        </label>
                                    </tr>
                                </tbody>
                            </table>
                        ...
                        <div class="w-2/5 min-h-[300px] rounded-lg overflow-hidden border border-black">
                            <table class="w-full text-center">
                                ...
                                <tbody>
                                    <tr>
                                        <label v-for="item in curriculumlist" :for="item.id" :key="item.id" class="grid grid-cols-2 border-b">
                                            <td :class="{'bg-blue-500' : !item.select}" class="flex justify-center items-center col-span-1">{{ item.course.course_name }}
                                            </td>
                                            <td :class="{'bg-blue-500' : !item.select}" class="flex justify-center items-center col-span-1">{{ item.teacher_name }}
                                            </td>
                    ...
                    <div class="flex justify-center gap-2">
                        ...
                        <button type="button" class="p-3 rounded-lg text-white bg-blue-500" @click="curriculumSave">儲存</button>
                    </div>
                </div>
            </div>
        </div>
